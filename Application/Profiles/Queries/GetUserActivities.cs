...
public class GetUserActivities
{
    ...
    public class Handler(AppDbContext context, IMapper mapper) : IRequestHandler<Query, Result<List<UserActivityDto>>>
    {
        ...
        {
            var query = context.ActivityAttendees.Where(u => u.UserId == request.UserId).OrderBy(a => a.Activity.Date).Select(x => x.Activity).AsQueryable();
            ...
            var activities = await projectedActivities.ToListAsync();
