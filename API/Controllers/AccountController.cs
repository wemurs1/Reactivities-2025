...
public class AccountController(SignInManager<User> signInManager, IEmailSender<User> emailSender, IConfiguration config) : BaseApiController
{
    ...
    public async Task<ActionResult> LoginWithGitHub(string code)
    {
        ...
        var tokenResponse = await httpClient.PostAsJsonAsync("https://github.com/login/oauth/access_token", new GitHubAuthRequest
        {
            Code = code,
    ...
    {
        if (string.IsNullOrEmpty(email) && string.IsNullOrEmpty(userId)) return BadRequest("Email or UserId must be provided");
